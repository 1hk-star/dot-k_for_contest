...
//        Log.d("private send() : ", "send Data ( \""+data.toString()+"\")");
        Log.d("private send() : ", "send Data ( \""+Integer.toString((int)data[0])+" "+Integer.toString((int)data[1])+" "+Integer.toString((int)data[2])+" "+Integer.toString((int)data[3])+"\")");
        if (tx == null || data == null || data.length == 0) {
            // Do nothing if there is no connection or message to send.
            if(tx == null) {
                Log.d("private send()", "no connection");
            }
            if(data == null || data.length==0) {
                Log.d("private send()", "no data to send");
            }
            return;
        }
    ...
    void sendByte(byte[] data){
        Log.d("sendByte() : ", "send Data ( \""+Integer.toString((int)data[0])+" "+Integer.toString((int)data[1])+" "+Integer.toString((int)data[2])+" "+Integer.toString((int)data[3])+"\")");
        send(data);
    }
    void send(String data) {
        Log.d("send() : ", "send Data ( \""+data+"\"");
